...
`.build()` returns an iterator of per-item `statum::Result<machine::SomeState>` — one result per row.
...
let machines: Vec<task_machine::SomeState> = results
    .into_iter()
    .filter_map(Result::ok)
    .collect();
```

If any validator is `async`, add `.await` after `.build()`:
...
Like `.into_machines()`, this returns one `statum::Result` per item:
...
Batch rebuilds preserve per-item failure information instead of forcing one all-or-nothing result shape. You can fail fast, collect only valid machines, or report partial errors per row.
